...
ApplicationWindow {
    ...
    ListView {
        ...
        Component.onCompleted: {
            ...
            for (var i in data["hardwares"]) {
                model.append({name: data["hardwares"][i]["name"]})
            }
        }
    }
    ...
    ListView {
        ...
        function setHardwareSpecificModelByIndex(index) {
            model.clear()
            var data = hardwareListReader.readJsonFile()["hardwares"][index]["specifics"]
           // console.log(index)
            //console.log(JSON.stringify(data))
            for (var i in data) {
                //console.log("setting specifics ", i, ", ", data[i]["name"])
                model.append({name: data[i]["name"], functions: data[i]["functions"]})
            }
        }
    }
}
